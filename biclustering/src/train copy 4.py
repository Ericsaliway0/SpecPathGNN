...
def train_and_evaluate(args, G_dgl, node_features):
    ...
    output_path = "results/leiden_visualization"
    os.makedirs(output_path, exist_ok=True)

    # saliency_pathway_matrix: np.ndarray, shape (num_pathways, num_genes)
    # gene_names: list of str
    # pathway_names: list of str
    # gene_clusters: dict, key=gene_name, value=cluster_id
    # pathway_clusters: dict, key=pathway_name, value=cluster_id

    # --- 1. Heatmap of gene × pathway saliency ordered by Leiden clusters ---
    # def plot_leiden_saliency_heatmap_(saliency_matrix, gene_names, pathway_names, gene_clusters, pathway_clusters, output_path):
    #     gene_order = sorted(gene_names, key=lambda g: gene_clusters[g])
    #     pathway_order = sorted(pathway_names, key=lambda p: pathway_clusters[p])
    #     X_ordered = saliency_matrix[[pathway_names.index(p) for p in pathway_order], :][:, [gene_names.index(g) for g in gene_order]]

    #     plt.figure(figsize=(12, 10))
    #     sns.heatmap(X_ordered, cmap="viridis", cbar_kws={"label": "Saliency"})
    #     plt.xlabel("Genes (Leiden ordered)")
    #     plt.ylabel("Pathways (Leiden ordered)")
    #     plt.tight_layout()
    #     plt.savefig(os.path.join(output_path, "leiden_saliency_heatmap.png"), dpi=300)
    #     plt.close()
    ...
    # # --- 2. Top genes per cluster ---
    # def plot_top_genes_per_cluster(gene_saliency, gene_names, gene_clusters, output_path, top_k=10):
    #     df = pd.DataFrame({
    #         "gene": gene_names,
    #         "saliency": gene_saliency,
    #         "cluster": [gene_clusters[g] for g in gene_names]
    #     })
    ...
    #         plt.tight_layout()
    #         plt.savefig(os.path.join(output_path, f"top_genes_cluster_{c}.png"), dpi=300)
    #         plt.close()

    # gene_saliency_np = saliency_pathway_matrix.sum(axis=0)
...
def plot_leiden_saliency_heatmap(
    ...
    ax = sns.heatmap(
        ...
        xticklabels=False,
        yticklabels=False,
        cbar_kws={"label": "Saliency (power-normalized)"}
    )

    # ---- Draw Leiden block boundaries ----
    def draw_boundaries(labels, axis="x"):
        prev = labels[0]
        pos = 0
        for i, lbl in enumerate(labels):
            if lbl != prev:
                if axis == "x":
                    ax.axvline(i, color="white", lw=0.5, alpha=0.6)
                else:
                    ax.axhline(i, color="white", lw=0.5, alpha=0.6)
                prev = lbl

    draw_boundaries([gene_clusters[gene_names[i]] for i in gene_order], "x")
    draw_boundaries([pathway_clusters[pathway_names[i]] for i in pathway_order], "y")

    # ---- Labels ----
    plt.xlabel("Genes (Leiden ordered)")
    plt.ylabel("Pathways (Leiden ordered)")
    plt.title("Gene–Pathway Saliency Map with Leiden Modules")
