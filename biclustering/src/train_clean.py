...
def train_and_evaluate(args, G_dgl, node_features):
    ...
    ########################################
    # 8. EDGE-LEVEL INTEGRATED GRADIENTS
    ########################################
    ...
    edge_ig = edge_ig.detach().cpu().numpy()

    # Min–max normalization
    edge_ig_norm = (edge_ig - edge_ig.min()) / (
        edge_ig.max() - edge_ig.min() + 1e-9
    )

    np.save(
        os.path.join(output_path, "edge_ig_normalized.npy"),
        edge_ig_norm
    )


    ########################################
    # 8C. TOP-K EDGES PER NODE
    ########################################

    top_k = 20
    edge_list = list(zip(u_all.cpu().numpy(),
                        v_all.cpu().numpy(),
                        edge_ig_norm))
    ...
    sns.heatmap(
        saliency_reordered,
        cmap="Reds",
        yticklabels=pathways_reordered,
        xticklabels=False
    )

    plt.xlabel("Genes (biclustered)")
    plt.ylabel("Pathways (biclustered)")
    plt.title("Saliency × Pathway Enrichment (Biclustered)")

    plt.tight_layout()
    plt.savefig(
        os.path.join(output_path, "saliency_pathway_heatmap_biclustered.png"),
        dpi=300
    )
    plt.close()


    def draw_cluster_boundaries(ax, labels, orientation="row"):
        """Draw lines between bicluster blocks."""
        changes = np.where(np.diff(labels) != 0)[0] + 1

        for c in changes:
            if orientation == "row":
                ax.hlines(c, *ax.get_xlim(), colors="black", linewidth=0.6)
            else:
                ax.vlines(c, *ax.get_ylim(), colors="black", linewidth=0.6)
    ...
    draw_cluster_boundaries(ax, row_labels[row_order], "row")
    draw_cluster_boundaries(ax, col_labels[col_order], "col")

    ax.set_xlabel("Genes (biclustered)")
    ax.set_ylabel("Pathways (biclustered)")

    plt.tight_layout()
    plt.savefig(
        os.path.join(output_path, "saliency_pathway_heatmap_biclustered_blocks.png"),
        dpi=300
    )
    plt.close()
    ...
    # assert saliency_pathway_matrix.shape[0] == len(bicluster_patient_ids)
    # assert len(bicluster.row_labels_) == len(bicluster_patient_ids)
    ...
    # Gene → Pathway-cluster edges
    df_gp = (
        df.groupby(["gene_cluster", "pathway_cluster"], as_index=False)
        .agg(weight=("weight", "sum"))
    )

    # Pathway-cluster → Family edges
    df_pf = (
        df.groupby(["pathway_cluster", "family"], as_index=False)
        .agg(weight=("weight", "sum"))
    )
    ...
    def plot_km(df, cluster):
        ...
        plt.tight_layout()
        plt.savefig(
            os.path.join(km_dir, f"KM_{cluster}.pdf")
        )
        plt.close()
