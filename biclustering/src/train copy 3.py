...
def train_and_evaluate(args, G_dgl, node_features):
    ...
    ########################################
    # 1. Train GNN (link prediction backbone)
    ########################################
    u, v = G_dgl.edges()
    ...
    train_g = dgl.remove_edges(
        G_dgl, eids[:test_size + val_size]
    )
    ...
    for epoch in range(args.epochs):
        ...
        loss = h.mean()  # placeholder loss (as in your current setup)
    ...
    model.eval()
    with torch.no_grad():
        embeddings = model(G_dgl, G_dgl.ndata["feat"]).cpu().numpy()

    np.save(
        os.path.join(output_path, "node_embeddings.npy"),
        embeddings
    )

    ########################################
    # 3. Compute gene-level saliency
    ########################################
    ...
    # Gene saliency = sum over feature dimensions
    gene_saliency = relevance.sum(dim=1).cpu().numpy()

    np.save(
        os.path.join(output_path, "gene_saliency.npy"),
        gene_saliency
    )

    ########################################
    # 4. Build Gene × Pathway enrichment matrix
    ########################################
    ...
    for _, r in df_enrich.iterrows():
        enrich_matrix[
            gene_to_idx[r["Gene2"]],
            pathway_to_idx[r["PathwayB"]]
        ] = r["enrich_score"]

    np.save(
        os.path.join(output_path, "enrichment_matrix.npy"),
        enrich_matrix
    )

    print(f"✅ Enrichment matrix shape: {enrich_matrix.shape}")

    ########################################
    # 5. Derive pathway saliency
    ########################################
    # Pathway saliency = S_g^T × E
    pathway_saliency = gene_saliency @ enrich_matrix
    ...
    np.save(
        os.path.join(output_path, "pathway_saliency.npy"),
        pathway_saliency
    )

    ########################################
    # 6. Gene × Pathway saliency matrix
    ########################################
    # R_{g,p} = S_g(g) × E_{g,p}
    gene_pathway_saliency = (
        gene_saliency[:, None] * enrich_matrix
    )

    np.save(
        os.path.join(output_path, "gene_pathway_saliency.npy"),
        gene_pathway_saliency
    )

    ########################################
    # 7. Save pathway embeddings (optional)
    ########################################
    np.save(
        os.path.join(output_path, "pathway_embeddings.npy"),
        embeddings
    )

    print("✅ Training and explainability pipeline complete")

    return {
        "gene_saliency": gene_saliency,
        "pathway_saliency": pathway_saliency,
        "gene_pathway_saliency": gene_pathway_saliency,
    ...
    # -------------------------------
    # 4. Gene labels (driver / non-driver / unknown)
    # -------------------------------
    d_set = set(pd.read_table("data/796_drivers.txt", header=None)[0].astype(str))
    nd_set = set(pd.read_table("data/2187_nondrivers.txt", header=None)[0].astype(str))

    gene_labels = [
        "Driver" if g in d_set else "NonDriver" if g in nd_set else "Unknown"
        for g in gene_names
    ]
    ...
    # 5. Plot UMAP
    # -------------------------------
    plot_gene_umap(embeddings[:len(gene_names)], gene_labels, output_path)
    plot_joint_gene_pathway_umap(
    ...
    # 6. Biclustering
    # -------------------------------
    saliency_pathway_matrix = build_saliency_pathway_matrix(
        saliency_pathway,
        enrich_matrix,
        gene_to_idx,
        pathway_to_idx
    )
    row_labels, col_labels = run_biclustering(saliency_pathway_matrix, output_path)
...
def preprocess_expression(expr_df, surv, gene_names):
    expr_df.columns = [c[:12] for c in expr_df.columns]
    expr_df = expr_df.groupby(expr_df.columns, axis=1).mean()

    patient_ids = sorted(set(expr_df.columns).intersection(surv.index))
...
def plot_gene_pathway_modules(df_enrich, output_path):
    import os
    os.makedirs(output_path, exist_ok=True)
    from some_module import build_and_plot_gene_pathway_modules, build_and_plot_gene_pathway_umap, plot_pathway_enrichment_dotplot
