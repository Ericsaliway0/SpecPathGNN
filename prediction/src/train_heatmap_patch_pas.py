...
def train_and_evaluate(args, G_dgl, node_features):
    ...
    plot_umap_with_cluster_colors(
        ...
        title="Gene UMAP (saliency-based)",
    )
    ...
    plot_umap_with_cluster_colors(
        ...
        title="Pathway UMAP (saliency-based)",
        point_size=30,
    )
...
def plot_umap_with_cluster_colors(
    ...
    legend_patches = [
        Patch(facecolor=cluster_colors[c], edgecolor="none", label=str(c))
    ...
    plt.legend(
        ...
        fontsize=9,
        title_fontsize=10,
        ncol=min(6, len(unique_clusters)),
    )
...
def plot_leiden_saliency_heatmap(
    ...
    """
    Plot a gene–pathway saliency heatmap with optional Leiden sorting and
    colored axis bars (no borders, no tick labels).

    Parameters
    ----------
    saliency_pathway_matrix : np.ndarray (n_pathways, n_genes)
    gene_names : list[str]
    pathway_names : list[str]
    gene_clusters : dict[str, int]
    pathway_clusters : dict[str, int]
    output_path : str
    vmax_percentile : float
        Upper percentile for color scaling.
    sort_by_cluster : bool
        If True, sort genes/pathways by Leiden cluster ID.
        If False, keep original input order.
    gamma : float
        PowerNorm gamma to boost low saliency values.
    cmap : str
    ...
    # -----------------------------
    # 3. Build cluster color bars (order-preserving)
    # -----------------------------
    ...
    def make_cluster_cmap(cluster_ids):
        ...
        for c in cluster_ids:
            if c not in unique:
                unique.append(c)
        palette = sns.color_palette("tab20", len(unique))
    ...
    gene_colors = np.array([gene_cmap[c] for c in gene_cluster_ids])
    pathway_colors = np.array([pathway_cmap[c] for c in pathway_cluster_ids])

    # -----------------------------
    # 4. Layout with GridSpec
    # -----------------------------
    ...
    ax_left = fig.add_subplot(gs[1, 0])   # pathway color bar
    ax_top  = fig.add_subplot(gs[0, 1])   # gene color bar
    ...
    xlabel = "Genes (Leiden ordered)" if sort_by_cluster else "Genes"
    ylabel = "Pathways (Leiden ordered)" if sort_by_cluster else "Pathways"

    ax_main.set_xlabel(xlabel, fontsize=18, labelpad=10)
    ax_main.set_ylabel(ylabel, fontsize=18, labelpad=10)
    ax_main.set_xticks([])
    ax_main.set_yticks([])
    ax_main.grid(False)
