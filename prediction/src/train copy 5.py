...
def train_and_evaluate(args, G_dgl, node_features):
    ...
    colors = [node_color(l) for l in labels]

    fig.data[0].node.color = colors


    png_path = os.path.join(output_path, "sankey_gene_pathway_family.png")

    fig.write_image(
        png_path,
        width=1400,
        height=800,
        scale=2   # ↑ higher = sharper (recommended for papers)
    )

    print(f"✅ Sankey saved to {png_path}")
