...
def train_and_evaluate(args, G_dgl, node_features):
    ...
    # -------------------------------
    surv = pd.read_csv("../ACGNN/data/TCGA-BRCA.survival.tsv", sep="\t")
    print("../ACGNN/data/TCGA-BRCA.survival.tsv-------------------",surv.head())
    print(surv.columns)
    # Standardize survival columns
    # surv = surv.rename(columns={"OS.time": "time", "OS": "event"})
    # surv["time"] = pd.to_numeric(surv["time"], errors="coerce")
    # surv["event"] = pd.to_numeric(surv["event"], errors="coerce")
    # surv = surv.dropna(subset=["time", "event"])

    # -------------------------------
    # 2. Load / preprocess expression data
    # -------------------------------
    expr_matrix, expr_df, surv, patient_ids, common_genes, gene_to_expr_idx = preprocess_expression(
        "../ACGNN/data/TCGA-BRCA.expression.tsv",
        surv,
        gene_names
    )
    ...
    # -------------------------------
    # 3. Compute patient cluster scores
    # -------------------------------
    patient_cluster_scores = compute_patient_cluster_scores(
        saliency_pathway_matrix,
        row_labels,
        gene_names,
        gene_to_expr_idx,
    ...
    patient_cluster_scores = patient_cluster_scores.loc[surv.index]

    surv = surv.set_index("_PATIENT")
    patient_cluster_scores.index  # check these match the same patient IDs


    # -------------------------------
    # 4. Merge cluster scores into survival dataframe
    # -------------------------------
    df_surv = surv.join(patient_cluster_scores, how="inner")

    print("Columns available for KM plotting:", df_surv.columns.tolist())

    # -------------------------------
    # 5. Optional: add global risk score
    # -------------------------------
    df_surv["risk_score"] = patient_cluster_scores.mean(axis=1)

    # -------------------------------
    # 6. Global KM plot
    # -------------------------------
    print(df_surv[['time', 'event']].head(10))
    print(df_surv[['time', 'event']].dtypes)

    plot_global_km(df_surv, output_path=os.path.join(output_path, "global_km.png"))

    # -------------------------------
    # 7. High vs Low KM per cluster
    # -------------------------------
    ...
    for c in cluster_cols:
        plot_high_low_km(
            df_surv=df_surv,
            risk_score_col=c,          # use the cluster column
            time_col="time",
            event_col="event",
            output_dir=os.path.join(output_path, "survival"),
            filename_prefix=f"KM_{c}_high_vs_low"
        )
...
def get_cluster_col(df, cluster_id):
    candidates = [f"Cluster_{cluster_id}", f"Cluster{cluster_id}"]
    for c in candidates:
        if c in df.columns:
            return c
    raise ValueError(f"Cluster column not found for cluster {cluster_id}")

def plot_global_km_(df_surv, output_path=None):
    """
    ...
    plt.show()
def plot_global_km(df_surv, output_path=None):
    """
    Plot Kaplan-Meier curve for all patients.
    df_surv must have columns ['time', 'event'].
    """
    df = df_surv.copy()
    ...
    # Convert event to int (0/1)
    df['event'] = df['event'].astype(int)

    if df.empty:
        raise ValueError("No valid rows in df_surv after converting 'time' and 'event' to numeric.")
...
def plot_high_low_km(
    df_surv,
    risk_score_col="Cluster_3",
    time_col="time",
    event_col="event",
    output_path=None,
    filename_prefix="KM_Cluster_3",
):
    """
    Plot High vs Low risk Kaplan–Meier curves with HR and log-rank p-value.

    Parameters
    ----------
    df_surv : pd.DataFrame
        Survival dataframe containing time, event, and risk score columns.
    risk_score_col : str
        Column used to stratify patients (e.g. GNN risk score).
    time_col : str
        Survival time column (default: 'OS.time').
    event_col : str
        Event indicator column (default: 'OS').
    output_path : str or None
        Directory to save plots (PNG + SVG). If None, does not save.
    filename_prefix : str
    ...
    # 1. Sanity checks
    # --------------------------------------------------
    print('df_surv.columns==',df_surv.columns)
    required = [time_col, event_col, risk_score_col]
    missing = [c for c in required if c not in df_surv.columns]
    if missing:
        raise ValueError(f"Missing columns in df_surv: {missing}")

    df = df_surv.copy()

    # --------------------------------------------------
    # 2. Ensure numeric survival columns
    # --------------------------------------------------
    df["time"] = pd.to_numeric(df[time_col], errors="coerce")
    df["event"] = pd.to_numeric(df[event_col], errors="coerce")
    df["risk"] = pd.to_numeric(df[risk_score_col], errors="coerce")

    df = df.dropna(subset=["time", "event", "risk"])

    # --------------------------------------------------
    # 3. Median stratification
    # --------------------------------------------------
    ...
    kmf_high.fit(
        df.loc[mask_high, "time"],
        df.loc[mask_high, "event"],
        label="High risk"
    )

    kmf_low.fit(
        df.loc[mask_low, "time"],
        df.loc[mask_low, "event"],
        label="Low risk"
    )
    ...
    # --------------------------------------------------
    # 6. Hazard ratio (Cox PH)
    # --------------------------------------------------
    cox_df = df[["time", "event", "group"]].copy()
    cox_df["group"] = (cox_df["group"] == "High").astype(int)
    ...
    hr_ci = np.exp(cph.confidence_intervals_.loc["group"])
    ...
    ax.set_title("High vs Low Risk Survival", fontsize=15)
    ax.grid(False)

    # --------------------------------------------------
    # 8. Stats annotation (bottom-left)
    # --------------------------------------------------
    stats_text = (
        f"HR = {hr:.2f} "
    ...
    if output_path is not None:
        ...
        plt.savefig(
            os.path.join(output_path, f"{filename_prefix}.png"),
            dpi=300,
            bbox_inches="tight"
        )

        plt.savefig(
            os.path.join(output_path, f"{filename_prefix}.svg"),
            bbox_inches="tight"
        )
...
def plot_leiden_saliency_heatmap_white_backgound(
    saliency_pathway_matrix,
    gene_names,
    pathway_names,
    gene_clusters,
