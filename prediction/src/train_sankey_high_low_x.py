...
def train_and_evaluate(args, G_dgl, node_features):
    ...
    cluster_id = 3
    group_col = f"Cluster{cluster_id}_group"

    df_gp_high, df_pf_high = build_group_sankey(
        ...
        gene_clusters=gene_clusters,
        pathway_clusters=pathway_clusters,
        PATHWAY_FAMILY_MAP=PATHWAY_FAMILY_MAP,
    )

    df_gp_low, df_pf_low = build_group_sankey(
        expr_matrix=expr_matrix,
        patient_ids=patient_ids,
        gene_to_expr_idx=gene_to_expr_idx,
        df_surv=df_surv,
...
def build_group_sankey(
    df_surv,
    group_col,      # e.g. "Cluster3_group"
    group_name,     # "High" or "Low"
    expr_matrix,
    patient_ids,
    gene_names,
    pathway_names,
...
def plot_side_by_side_sankey(
    ...
    def build_edges(df_gp, df_pf):
        edges = []
        for _, r in df_gp.iterrows():
            edges.append((r["gene_cluster"], r["pathway_cluster"], r["weight"]))
        for _, r in df_pf.iterrows():
            edges.append((r["pathway_cluster"], r["family"], r["weight"]))
        return edges

    edges_high = build_edges(df_gp_high, df_pf_high)
    edges_low  = build_edges(df_gp_low, df_pf_low)

    all_labels = sorted(
        set([e[0] for e in edges_high + edges_low] +
            [e[1] for e in edges_high + edges_low])
    )

    label_to_id = {l: i for i, l in enumerate(all_labels)}
    colors = [sankey_node_color(l) for l in all_labels]

    def sankey_trace(edges, title):
        return go.Sankey(
            arrangement="snap",
            node=dict(
                label=all_labels,
            ...
            link=dict(
                source=[label_to_id[e[0]] for e in edges],
                target=[label_to_id[e[1]] for e in edges],
    ...
    fig = make_subplots(
        ...
        subplot_titles=("High-risk patients", "Low-risk patients"),
    )

    fig.add_trace(sankey_trace(edges_high, "High"), row=1, col=1)
    fig.add_trace(sankey_trace(edges_low,  "Low"),  row=1, col=2)

    fig.update_layout(
        title="Gene → Pathway → Family flows by survival group",
        font_size=12,
        width=1600,
        height=700,
    )
    ...
    os.makedirs(output_path, exist_ok=True)
    fig.write_image(
        os.path.join(output_path, "sankey_high_vs_low.png"),
        scale=3
    )
