...
def train_no_visualization(hyperparams=None, data_path='embedding/data/emb', plot=True):
    """
    Train GAT on gene-level graphs.
    """
    ...
    model_path = os.path.join(model_dir, f'model_dim{out_feats}_lay{num_layers}_epo{num_epochs}.pth')
    results_path = os.path.join(data_path, 'results/gene_embeddings')
    ...
    ds_train, ds_valid = torch.utils.data.random_split(ds, [train_size, valid_size])
    ...
    criterion = FocalLoss(alpha=0.25, gamma=2.0, reduction='none')  # keep 'none' for weighting

    # --- Class weights ---
    weight_tensor = torch.tensor([0.00001, 0.99999]).to(device)
    ...
    # --- Load NetworkX graph for node order ---
    nx_graph = pickle.load(open(os.path.join(data_path, 'raw/emb_train.pkl'), 'rb'))
    nx_graph = nx_graph.graph_nx  # extract NetworkX graph
    node_list = list(nx_graph.graph_nx.nodes())  # node order
    print(f"✅ Network loaded with {len(node_list)} nodes")
    ...
    for epoch in tqdm(range(num_epochs), desc='Training', unit='epoch'):
        ...
        for graphs, _ in dl_train:
            graphs = graphs.to(device)
            logits = net(graphs).view(-1)  # [N]
            labels = graphs.ndata['significance'].float().view(-1)  # [N]

            # Apply weights per class
            sample_weights = weight_tensor[labels.long()]
            loss = criterion(logits, labels)
            loss_weighted = (loss * sample_weights).mean()
            ...
            train_losses.append(loss_weighted.item())
            train_f1s.append(f1)
        ...
        with torch.no_grad():
            for graphs, _ in dl_valid:
                graphs = graphs.to(device)
                logits = net(graphs).view(-1)
                labels = graphs.ndata['significance'].float().view(-1)
                ...
                preds = (torch.sigmoid(logits) > 0.5).int()
                f1 = metrics.f1_score(labels.cpu(), preds.cpu())
                valid_losses.append(loss_weighted.item())
                valid_f1s.append(f1)
    ...
    for idx, node in enumerate(node_list):
        embeddings_list.append({
            "GeneSymbol": str(node),  # node ID as gene symbol
            **{f"dim_{i}": float(val) for i, val in enumerate(all_embeddings[idx])}
        })

    df_embeddings = pd.DataFrame(embeddings_list)
    dim_cols = [c for c in df_embeddings.columns if c.startswith("dim_")]
    df_embeddings[dim_cols] = (df_embeddings[dim_cols] - df_embeddings[dim_cols].min()) / (
        df_embeddings[dim_cols].max() - df_embeddings[dim_cols].min()
    )

    embeddings_csv_path = os.path.join(results_path, 'gene_embeddings.csv')
    df_embeddings.to_csv(embeddings_csv_path, index=False)
    print(f"✅ Gene embeddings saved to {embeddings_csv_path}")
...
def collate_fn(batch):
    graphs, gene_lists = map(list, zip(*batch))
    batched_graph = dgl.batch(graphs)
    return batched_graph, gene_lists

# --- Training function ---
def train_(hyperparams=None, data_path='embedding/data/emb', plot=True):
    device = hyperparams['device']
    num_epochs = hyperparams['num_epochs']
    in_feats = hyperparams['in_feats']
    out_feats = hyperparams['out_feats']
    ...
    ds_train = dataset.GeneDataset(root=data_path)
    ds_valid = dataset.GeneDataset(root=data_path)  # adjust if you have a validation split

    dl_train = GraphDataLoader(ds_train, batch_size=batch_size, shuffle=True, collate_fn=collate_fn)
    dl_valid = GraphDataLoader(ds_valid, batch_size=batch_size, shuffle=False, collate_fn=collate_fn)
    ...
    best_model = copy.deepcopy(net)
    best_valid_loss = float('inf')
    results_path = os.path.join(data_path, 'results/node_embeddings')
    os.makedirs(results_path, exist_ok=True)
    ...
    for epoch in tqdm(range(num_epochs), desc="Training"):
        ...
        for g, batch_gene_lists in dl_train:
            g = g.to(device)
            logits = net(g).view(-1)
            # labels = g.ndata['significance'].unsqueeze(-1).float()
            labels = g.ndata['significance'].float()  # shape [N]
        ...
        loss_per_epoch_train.append(np.mean(train_losses))
        f1_per_epoch_train.append(np.mean(train_f1s))

        # --- Validation ---
        net.eval()
        val_losses, val_f1s = [], []
        with torch.no_grad():
            for g, batch_gene_lists in dl_valid:
                g = g.to(device)
                logits = net(g).view(-1)
                # labels = g.ndata['significance'].unsqueeze(-1).float()
                labels = g.ndata['significance'].float()  # shape [N]
        ...
        loss_per_epoch_valid.append(np.mean(val_losses))
        f1_per_epoch_valid.append(np.mean(val_f1s))

        if np.mean(val_losses) < best_valid_loss:
            best_valid_loss = np.mean(val_losses)
            best_model.load_state_dict(copy.deepcopy(net.state_dict()))

        print(f"Epoch {epoch+1}: Train Loss {loss_per_epoch_train[-1]:.4f}, F1 {f1_per_epoch_train[-1]:.4f} | "
              f"Valid Loss {loss_per_epoch_valid[-1]:.4f}, F1 {f1_per_epoch_valid[-1]:.4f}")
    ...
    for g, batch_gene_lists in dl_train:
        g = g.to(device)
        emb = best_model(g).detach().cpu()
        # Store embeddings by GeneSymbol
        # for i, gene_symbol in enumerate(sum(batch_gene_lists, [])):
        for i, gene_symbol in enumerate(batch_gene_lists):
            embedding_dict[gene_symbol] = emb[i].numpy()

    # --- Visualizations ---
    all_embeddings = np.array(list(embedding_dict.values()))
    stid_list = list(embedding_dict.keys())
    cluster_labels = calculate_cluster_labels(best_model, dl_train, device)[1]

    save_path_heatmap = os.path.join(results_path, 'heatmap.png')
    save_path_matrix = os.path.join(results_path, 'cos_sim_matrix.png')
    save_path_pca = os.path.join(results_path, 'pca.png')
    save_path_tsne = os.path.join(results_path, 'tsne.png')

    # create_heatmap_with_stid(all_embeddings, stid_list, save_path_heatmap)
    plot_cosine_similarity_matrix_for_clusters_with_values(all_embeddings, stid_list, save_path_matrix)
    visualize_embeddings_pca(all_embeddings, cluster_labels, stid_list, save_path_pca)
    visualize_embeddings_tsne(all_embeddings, cluster_labels, stid_list, save_path_tsne)

    # --- Save model ---
    model_dir = os.path.join(data_path, 'models')
    os.makedirs(model_dir, exist_ok=True)
    model_path = os.path.join(model_dir, f'model_dim{out_feats}_layers{num_layers}.pth')
    torch.save(best_model.state_dict(), model_path)

    return model_path, embedding_dict


# --- Training function ---
def train_f1_loss_visualization(hyperparams=None, data_path='embedding/data/emb', plot=True):
    device = hyperparams['device']
    num_epochs = hyperparams['num_epochs']
    in_feats = hyperparams['in_feats']
    out_feats = hyperparams['out_feats']
    ...
    for epoch in tqdm(range(num_epochs), desc="Training"):
        ...
        for g, batch_gene_lists in dl_train:
            g = g.to(device)
            logits = net(g).view(-1)
            labels = g.ndata['significance'].float()
        ...
        print(f"Epoch {epoch+1}: "
              f"Train Loss {loss_per_epoch_train[-1]:.4f}, F1 {f1_per_epoch_train[-1]:.4f} | "
              f"Valid Loss {loss_per_epoch_valid[-1]:.4f}, F1 {f1_per_epoch_valid[-1]:.4f}")
    ...
    if plot:
        ...
        # Loss plot
        plt.subplot(1, 2, 1)
        plt.plot(loss_per_epoch_train, label="Train Loss", marker='o')
        plt.plot(loss_per_epoch_valid, label="Valid Loss", marker='s')
        plt.xlabel("Epoch")
        plt.ylabel("Loss")
        ...
        # F1 plot
        plt.subplot(1, 2, 2)
        plt.plot(f1_per_epoch_train, label="Train F1", marker='o')
        plt.plot(f1_per_epoch_valid, label="Valid F1", marker='s')
        plt.xlabel("Epoch")
        plt.ylabel("F1 Score")
...
def train(hyperparams=None, data_path='embedding/data/emb', plot=True):
    ...
    # # === Save Pretrained Embeddings to CSV ===
    # embeddings_csv_path = os.path.join(results_path, f'embeddings_head{num_heads}_dim{out_feats}_lay{num_layers}_epo{num_epochs}.csv')

    # # Collect embeddings into dataframe
    # embeddings_list = []
    # for stId, emb in stid_dic.items():
    #     embeddings_list.append({
    #         "stId": stId,
    #         **{f"dim_{i}": float(val) for i, val in enumerate(emb)}
    #     })
    ...
    # === Save Pretrained Embeddings to CSV (with pathway + mapped genes) ===


    # embeddings_csv_path = os.path.join(
    #     results_path,
    #     f'embeddings_head{num_heads}_dim{out_feats}_lay{num_layers}_epo{num_epochs}.csv'
    # )

    # # Load pathway → gene mapping
    # mapping_file = "data/pathways_mapped_all_genes.tsv"
    # mapping_df = pd.read_csv(mapping_file, sep="\t")

    # # Build dictionary: pathway → list of genes
    # pathway_to_genes = {}
    # for _, row in mapping_df.iterrows():
    #     pathway_id = row["PathwayID"]
    #     genes = []
    #     for col in row.index:
    #         if col.startswith("Gene") and pd.notna(row[col]) and str(row[col]).strip():
    #             split_genes = [g.strip() for g in str(row[col]).split(",") if g.strip()]
    #             genes.extend(split_genes)
    #     if genes:
    #         pathway_to_genes[pathway_id] = genes

    # # Collect embeddings into dataframe (skip if no mapping exists)
    # embeddings_list = []
    # for stId, emb in stid_dic.items():
    #     if stId not in pathway_to_genes:
    #         continue  # skip unmapped pathways
    #     for gene in pathway_to_genes[stId]:
    #         embeddings_list.append({
    #             "PathwayID": stId,
    #             "Gene": gene,
    #             **{f"dim_{i}": float(val) for i, val in enumerate(emb)}
    ...
    # # === Normalize embeddings into [0, 1] range ===
    # dim_cols = [c for c in df_embeddings.columns if c.startswith("dim_")]
    # df_embeddings[dim_cols] = (df_embeddings[dim_cols] - df_embeddings[dim_cols].min()) / (
    #     df_embeddings[dim_cols].max() - df_embeddings[dim_cols].min()
    # )
    ...
    # print(f"✅ Normalized embeddings (Pathway + all mapped genes) saved to {embeddings_csv_path}")
...
def plot_cosine_similarity_matrix_for_clusters_with_values_(embeddings, stids, save_path):
    # Ensure embeddings is 2D
    embeddings = np.array(embeddings)
    if embeddings.ndim == 1:
        embeddings = embeddings.reshape(-1, 1)  # (N, 1)

    # Compute cosine similarity
    cos_sim = np.dot(embeddings, embeddings.T)
    norms = np.linalg.norm(embeddings, axis=1, keepdims=True)
    cos_sim /= np.dot(norms, norms.T)
    ...
    ax = sns.heatmap(
        cos_sim, cmap="Spectral", annot=True, fmt=".3f", annot_kws={"size": 6},
        xticklabels=stids, yticklabels=stids,
        cbar_kws={"shrink": 0.2, "aspect": 15, "ticks": [vmin, vmax]}
    )

    # Highlight diagonal (self-similarity = 1)
    for i in range(len(stids)):
        ax.add_patch(plt.Rectangle((i, i), 1, 1, fill=True, color='black', alpha=0.5, zorder=3))
...
def calculate_cluster_labels(net, dataloader, device, num_clusters=12):
    all_embeddings = []
    net.eval()
    with torch.no_grad():
