...
def train(hyperparams=None, data_path='gnn_embedding/data/emb', plot=True):
    ...
    contrastive_weight = hyperparams.get('contrastive_weight', 5.0)
    ...
    # Analyze labeled vs. unlabeled nodes
    results_path = 'gnn_embedding/results/node_embeddings/'
    os.makedirs(results_path, exist_ok=True)
    analyze_label_distribution(ds_train[0][0], "Train", results_path)
    analyze_label_distribution(ds_valid[0][0], "Test", results_path)
    ...
    # -------------------------
    # Save embeddings (before & after) for downstream analysis
    # -------------------------
    emb_before_path = os.path.join(results_path, f"emb_before_dim{out_feats}_lay{num_layers}_epo{num_epochs}.npy")
    emb_after_path  = os.path.join(results_path, f"emb_after_dim{out_feats}_lay{num_layers}_epo{num_epochs}.npy")



    #######################################################################################################################
    
    # all_embeddings_initial and all_embeddings exist above
    np.save(emb_before_path, all_embeddings_initial)
    np.save(emb_after_path, all_embeddings)
    print(f"✅ Saved embeddings: {emb_before_path}, {emb_after_path}")
    ...
    # # emb_after_path = os.path.join(results_path, "emb_after.npy")
    # np.save(emb_after_path, all_embeddings)
    ...
    gcl_eval_radar_file = os.path.join(results_path, f"gcl_eval_radar_with_ttest_dim{out_feats}_lay{num_layers}_epo{num_epochs}.png")
    gcl_eval_csv_path = os.path.join(results_path, f"gcl_eval_csv_dim{out_feats}_lay{num_layers}_epo{num_epochs}.csv")
    
    metrics, sil_before, sil_after, db_before, db_after= evaluate_gcl(
        emb_before=all_embeddings_initial,
        emb_after=all_embeddings,
        cluster_labels=cluster_labels,
        node_labels=None,
        radar_file=gcl_eval_radar_file,
        csv_file=gcl_eval_csv_path,
        num_epochs=num_epochs
    )

    # metrics, sil_before, sil_after, db_before, db_after = evaluate_gcl(emb_before=all_embeddings_initial, 
    #                                                                    emb_after=all_embeddings, 
    #                                                                    cluster_labels=cluster_labels, 
    #                                                                    node_labels=None, 
    #                                                                    radar_file=gcl_eval_radar_file, 
    ...
    gcl_eval_summar_path = os.path.join(results_path, f"gcl_eval_summary_dim{out_feats}_lay{num_layers}_epo{num_epochs}.csv")
    pd.Series(metrics).to_csv(gcl_eval_summar_path)
    print(metrics)
    ...
    def compute_embedding_quality_metrics(embeddings, labels):
        ...
        for cluster in np.unique(labels):
            ...
            others_idx = np.where(labels != cluster)[0]
            intra = dist_matrix[np.ix_(cluster_idx, cluster_idx)]
            inter = dist_matrix[np.ix_(cluster_idx, others_idx)]
            if len(cluster_idx) > 1:
                intra_dists.extend(intra[np.triu_indices_from(intra, 1)])
            inter_dists.extend(inter.flatten())

        return {
            "Silhouette Score": silhouette,
            "Davies–Bouldin Index": db,
            "Intra-cluster Similarity": np.mean(intra_dists),
            "Inter-cluster Similarity": np.mean(inter_dists),
            "Intra Distribution": intra_dists,
            "Inter Distribution": inter_dists,
        }

    n_clusters = 10
    kmeans_before = KMeans(n_clusters=n_clusters, random_state=42).fit(all_embeddings_initial)
    kmeans_after = KMeans(n_clusters=n_clusters, random_state=42).fit(all_embeddings)
    labels_before, labels_after = kmeans_before.labels_, kmeans_after.labels_

    metrics_before = compute_embedding_quality_metrics(all_embeddings_initial, labels_before)
    metrics_after = compute_embedding_quality_metrics(all_embeddings, labels_after)

    # -------------------------------------------
    # Radar plot + Dashboard
    # -------------------------------------------
    def normalize_metric(value, lower_better=False):
        if lower_better:
            return 1 - (value / (value + 1e-8))
        return value / (value + 1e-8)

    metric_names = ["Silhouette Score", "Davies–Bouldin Index", "Intra-cluster Similarity", "Inter-cluster Similarity"]
    before_values = [normalize_metric(metrics_before[m], "Davies" in m or "Intra" in m) for m in metric_names]
    after_values = [normalize_metric(metrics_after[m], "Davies" in m or "Intra" in m) for m in metric_names]
    ...
    # # Radar plot
    # radar_path = os.path.join(results_path, f'gcl_embedding_quality_radar_dim{out_feats}_lay{num_layers}_epo{num_epochs}.png')

    # plt.figure(figsize=(7, 6))
    # ax = plt.subplot(111, polar=True)
    # ax.set_theta_offset(np.pi / 2)
    # ax.set_theta_direction(-1)
    # plt.xticks(angles[:-1], metric_names, fontsize=11)
    # ax.plot(angles, before_values, linewidth=2, linestyle='solid', label='Before GCL', color="#1f77b4")
    # ax.fill(angles, before_values, color="#1f77b4", alpha=0.15)
    # ax.plot(angles, after_values, linewidth=2.5, linestyle='solid', label='After GCL', color="#ff7f0e")
    # ax.fill(angles, after_values, color="#ff7f0e", alpha=0.25)
    # plt.title("Embedding Quality Metrics: Before vs. After GCL", size=14, pad=25)
    # plt.legend(loc='upper right', bbox_to_anchor=(1.3, 1.05), frameon=False, fontsize=11)
    # plt.tight_layout()
    # plt.savefig(radar_path, dpi=300, bbox_inches="tight")
    # plt.close()

    # Dashboard (bar + violin plots)
    dashboard_path = os.path.join(results_path, f'gcl_embedding_quality_dashboard_dim{out_feats}_lay{num_layers}_epo{num_epochs}.png')
    fig, axes = plt.subplots(1, 3, figsize=(15, 5))

    # Bar chart
    bar_labels = ['Silhouette Score (↑)', 'Davies–Bouldin Index (↓)']
    # before_bar = [metrics_before['Silhouette Score'], metrics_before['Davies–Bouldin Index']]
    before_bar = [sil_before, db_before]
    # after_bar = [sil_after, metrics_after['Davies–Bouldin Index']]
    after_bar = [sil_after, db_after]
    x = np.arange(len(bar_labels))
    width = 0.35
    axes[0].bar(x - width/2, before_bar, width, label='Before GCL', color="#1f77b4", alpha=0.8)
    axes[0].bar(x + width/2, after_bar, width, label='After GCL', color="#ff7f0e", alpha=0.8)
    axes[0].set_xticks(x)
    axes[0].set_xticklabels(bar_labels, rotation=20, ha='right')
    axes[0].set_ylabel('Score')
    axes[0].set_title('Clustering Metrics')
    axes[0].legend()

    # Intra violin
    sns.violinplot(data=[metrics_before["Intra Distribution"], metrics_after["Intra Distribution"]],
                   palette=["#1f77b4", "#ff7f0e"], ax=axes[1])
    axes[1].set_xticks([0, 1])
    axes[1].set_xticklabels(['Before GCL', 'After GCL'])
    axes[1].set_ylabel('Cosine Distance')
    axes[1].set_title('Intra-cluster Distance Distribution (↓)')

    # Inter violin
    sns.violinplot(data=[metrics_before["Inter Distribution"], metrics_after["Inter Distribution"]],
                   palette=["#1f77b4", "#ff7f0e"], ax=axes[2])
    axes[2].set_xticks([0, 1])
    axes[2].set_xticklabels(['Before GCL', 'After GCL'])
    axes[2].set_ylabel('Cosine Distance')
    axes[2].set_title('Inter-cluster Distance Distribution (↑)')

    plt.suptitle("Graph Contrastive Learning — Embedding Quality Dashboard", fontsize=15)
    plt.tight_layout(rect=[0, 0, 1, 0.96])
    plt.savefig(dashboard_path, dpi=300, bbox_inches="tight")
    plt.close()

    # print(f"✅ Saved radar plot: {radar_path}")
    print(f"✅ Saved dashboard: {dashboard_path}")

    # -------------------------
    # Compute cluster-wise intra/inter and global metrics
    # -------------------------

    # If reuse the cluster_labels computed for `all_embeddings_initial`,
    # ensure that the KMeans used below matches your prior clustering strategy.
    def compute_metrics(emb, n_clusters=10):
        km = KMeans(n_clusters=n_clusters, random_state=42).fit(emb)
        labels = km.labels_
        sil = silhouette_score(emb, labels)
        db = davies_bouldin_score(emb, labels)
        # pairwise cosine distances -> convert to similarities if you prefer similarity
        ...
        for c in np.unique(labels):
            ...
            if len(idx) > 1:
                ...
                # take upper triangle (exclude self-distances)
                tri_idx = np.triu_indices_from(intra, k=1)
                intra_list.extend(intra[tri_idx].tolist())
            if len(others) > 0:
                inter = dist[np.ix_(idx, others)]
                inter_list.extend(inter.flatten().tolist())
        intra_mean = float(np.mean(intra_list)) if len(intra_list) > 0 else float('nan')
        inter_mean = float(np.mean(inter_list)) if len(inter_list) > 0 else float('nan')
        return {
            "silhouette": float(sil),
            "db": float(db),
            "intra_mean": intra_mean,
            "inter_mean": inter_mean,
            "intra_samples": intra_list,
            "inter_samples": inter_list
        }

    # choose cluster count consistent with dashboard
    n_clusters = 10
    m_before = compute_metrics(all_embeddings_initial, n_clusters=n_clusters)
    m_after  = compute_metrics(all_embeddings, n_clusters=n_clusters)

    # -------------------------
    # Save metrics.csv (for the dashboard script)
    # -------------------------
    metrics_csv_path = os.path.join(results_path, "metrics.csv")
    metrics_rows = [
        ("silhouette_before", m_before["silhouette"]),
        ("silhouette_after", m_after["silhouette"]),
        ("db_before", m_before["db"]),
        ("db_after", m_after["db"]),
        ("intra_mean_before", m_before["intra_mean"]),
        ("intra_mean_after", m_after["intra_mean"]),
        ("inter_mean_before", m_before["inter_mean"]),
        ("inter_mean_after", m_after["inter_mean"]),
        ("intra_gain", float(m_after["intra_mean"] - m_before["intra_mean"])),
        ("inter_change", float(m_after["inter_mean"] - m_before["inter_mean"]))
    ]
    ...
    # # -------------------------
    # # Save raw intra/inter samples for violin plots
    # # -------------------------
    # np.save(os.path.join(results_path, "intra_before.npy"), np.array(m_before["intra_samples"]))
    # np.save(os.path.join(results_path, "intra_after.npy"),  np.array(m_after["intra_samples"]))
    # np.save(os.path.join(results_path, "inter_before.npy"), np.array(m_before["inter_samples"]))
    # np.save(os.path.join(results_path, "inter_after.npy"),  np.array(m_after["inter_samples"]))
    # print("✅ Saved intra/inter sample arrays for dashboard.")

    # -------------------------
    # Optional: call local dashboard generation functions (if available)
    # If you implemented evaluate_gcl_embedding_quality_dashboard.py as importable, call it here.
    # Otherwise, the files saved above (metrics.csv, emb_before/after.npy, ... ) are ready
    # for external scripts to generate plots.
    # Example:
    # from evaluate_gcl_embedding_quality_dashboard import make_dashboard_from_saved
    # make_dashboard_from_saved(results_path)
    # ------------------------- 
    ...
    # ================================================
    nx_graph = pickle.load(open(graph_path, 'rb'))
    gene_names = list(nx_graph.graph_nx.nodes)

    visualize_gcl_comparison(
        emb_before=all_embeddings_initial,
        emb_after=all_embeddings,
        cluster_labels=cluster_labels,
        umap_path = os.path.join(results_path, f'gcl_umap_dim{out_feats}_lay{num_layers}_epo{num_epochs}.png'),
        cos_path = os.path.join(results_path, f'gcl_cosine_dim{out_feats}_lay{num_layers}_epo{num_epochs}.png'),
        tsne_path = os.path.join(results_path, f'gcl_tsne_dim{out_feats}_lay{num_layers}_epo{num_epochs}.png'),
...
def analyze_label_distribution_no_csv_saving(graph, split_name, results_path):
    """
    Analyze how many nodes are labeled/unlabeled in a given graph.
    Saves a pie chart under results_path.
    """
    significance = graph.ndata.get("significance", None)
    if significance is None:
        print(f"⚠️ No 'significance' feature in graph for {split_name}. Skipping label analysis.")
        return

    # Count labeled vs unlabeled
    labeled_mask = (significance != 0) & (~torch.isnan(significance))
    num_labeled = labeled_mask.sum().item()
    num_unlabeled = (~labeled_mask).sum().item()
    total_nodes = len(significance)

    # Display info
    print(f"\n=== {split_name.upper()} LABEL ANALYSIS ===")
    print(f"🔹 Total nodes: {total_nodes}")
    print(f"✅ Labeled nodes: {num_labeled} ({num_labeled / total_nodes:.2%})")
    print(f"⚪ Unlabeled nodes: {num_unlabeled} ({num_unlabeled / total_nodes:.2%})")

    # --- Visualization ---
    os.makedirs(results_path, exist_ok=True)
    labels = ['Labeled', 'Unlabeled']
    sizes = [num_labeled, num_unlabeled]
    ...
    plt.title(f"{split_name.capitalize()} Graph — Node Label Availability", fontsize=13)
    plt.tight_layout()
    path = os.path.join(results_path, f"{split_name.lower()}_label_distribution_pie.png")
    plt.savefig(path, dpi=300)
    plt.close()

    print(f"📊 Saved {split_name} label distribution pie chart to: {path}")

def analyze_label_distribution(graph, split_name, results_path):
    """
    Analyze how many nodes are labeled/unlabeled in a given graph.
    Saves a pie chart and CSV summary under results_path.
    """
    import pandas as pd
    import torch
    import matplotlib.pyplot as plt
    ...
    pie_path = os.path.join(results_path, f"{split_name.lower()}_label_distribution_pie.png")
    ...
    csv_data = {
        ...
        "Labeled_Percentage": [f"{num_labeled / total_nodes:.2%}"],
        "Unlabeled_Percentage": [f"{num_unlabeled / total_nodes:.2%}"]
    }
    df = pd.DataFrame(csv_data)
    csv_path = os.path.join(results_path, f"{split_name.lower()}_label_distribution_summary.csv")
    df.to_csv(csv_path, index=False)

    print(f"📊 Saved {split_name} label distribution pie chart to: {pie_path}")
    print(f"📄 Saved {split_name} label distribution summary CSV to: {csv_path}")
...
def evaluate_gcl_ori(
    ...
    csv_file = 'gcl_eval_csv.csv',
    radar_file = "gcl_eval_radar_with_ttest.png",
    num_epochs=10,
    # output_dir="gnn_embedding/results/gcl_eval",
    filename=None
):
    """
    Evaluate contrastive learning improvement in graph embeddings.
    Performs cluster quality analysis, intra/inter similarity statistics,
    Welch’s t-test for intra-cluster improvement, linear separability tests,
    ...
    if cluster_labels is not None:
        ...
        # Welch’s t-test for intra-cluster cosine similarity (unpaired, unequal variance)
        try:
            m = min(len(ib_vals), len(ia_vals))
            if m > 10:
                t_stat, p_val = stats.ttest_ind(ia_vals[:m], ib_vals[:m], equal_var=False)
            else:
                t_stat, p_val = np.nan, np.nan

            results['intra_t_stat'] = float(t_stat) if not np.isnan(t_stat) else None

            # High precision for very small p-values
            # getcontext().prec = 50
            # results['intra_p_val'] = str(Decimal(p_val)) if not np.isnan(p_val) else None
            results['intra_p_val'] = f"{p_val:.6e}" if not np.isnan(p_val) else None
        ...
        # No clustering labels available — fallback to pairwise similarity means
    ...
    # ---- 3) Clustering agreement (optional ground-truth labels) ----
    ...
    if cluster_labels is not None:
        ...
        print("Intra Welch’s t-test p-value:", results.get('intra_p_val'))
    if node_labels is not None:
    ...
    # ---- 7) Radar plot (no normalization)
    try:
        radar_metrics = {
            "Silhouette": (results.get('silhouette_before'), results.get('silhouette_after')),
            "DB (1/x)": (
                None if results.get('db_before') in [None, 0] else 1.0 / results['db_before'],
                None if results.get('db_after') in [None, 0] else 1.0 / results['db_after']
            ),
            "Intra Mean": (results.get('intra_mean_before'), results.get('intra_mean_after')),
            "Inter Mean": (results.get('inter_mean_before'), results.get('inter_mean_after')),
            "Linear Acc": (results.get('linear_acc_before'), results.get('linear_acc_after')),
        }

        labels, before_vals, after_vals = [], [], []
        for k, (b, a) in radar_metrics.items():
            if b is not None and a is not None and not (np.isnan(b) or np.isnan(a)):
                labels.append(k)
        ...
        if len(labels) >= 3:
            ...
            # Plot radar
            ax.plot(angles, before_vals, label='Before GCL', color='skyblue', linewidth=2, marker='o')
            ax.fill(angles, before_vals, color='skyblue', alpha=0.25)
            ax.plot(angles, after_vals, label='After GCL', color='orange', linewidth=2, marker='o')
            ax.fill(angles, after_vals, color='orange', alpha=0.25)

            # Annotate values
            for i, angle in enumerate(angles[:-1]):
                ax.text(angle, after_vals[i]*1.05, f"{after_vals[i]:.3f}", color='orange', ha='center', va='center', fontsize=10)
                ax.text(angle, before_vals[i]*0.9, f"{before_vals[i]:.3f}", color='steelblue', ha='center', va='center', fontsize=10)

            # === Add Welch’s t-test p-value annotation on Intra Mean axis ===
            if "Intra Mean" in labels and results.get("intra_p_val") is not None:
                intra_idx = labels.index("Intra Mean")
                angle_intra = angles[intra_idx]
                p_text = f"Welch’s t-test\np = {results['intra_p_val']}"
                ax.text(angle_intra, (after_vals[intra_idx] + before_vals[intra_idx]) / 2,
                        p_text, fontsize=11, color='black', ha='center', va='center',
            ...
            ax.set_title("GCL Evaluation Radar Chart (with Welch’s t-test)", size=15, pad=20)

            all_vals = np.concatenate([before_vals, after_vals])
            vmin, vmax = np.min(all_vals), np.max(all_vals)
            margin = (vmax - vmin) * 0.2 if vmax - vmin > 0 else 0.1
            ax.set_ylim(vmin - margin, vmax + margin)
            ax.yaxis.grid(True, linestyle='dotted', color='gray', alpha=0.7)

            ax.legend(loc='upper right', bbox_to_anchor=(1.3, 1.1))
            # os.makedirs(output_dir, exist_ok=True)
            # radar_path = os.path.join(output_dir, "gcl_eval_radar_with_ttest_epo{num_epochs}.png")
            plt.savefig(radar_file, dpi=300, bbox_inches='tight')
            plt.close()
            print(f"✅ Radar plot with Welch’s t-test saved to: {radar_file}")
        else:
            print("⚠️ Not enough comparable metrics to plot radar chart.")
    except Exception as e:
        print(f"⚠️ Failed to plot radar chart: {e}")


    return results, results.get('silhouette_before'), results.get('silhouette_after'), results.get('db_before'), results.get('db_after')
...
def evaluate_gcl_(
    ...
    """
    Extended GCL evaluation with multiple clustering metrics and Welch’s t-test for all.
    """
    ...
    results['silhouette_before'] = safe_metric(silhouette_score, emb_before, cluster_labels)
    results['silhouette_after']  = safe_metric(silhouette_score, emb_after, cluster_labels)
    results['db_before'] = safe_metric(davies_bouldin_score, emb_before, cluster_labels)
    results['db_after']  = safe_metric(davies_bouldin_score, emb_after, cluster_labels)
    results['ch_before'] = safe_metric(calinski_harabasz_score, emb_before, cluster_labels)
    results['ch_after']  = safe_metric(calinski_harabasz_score, emb_after, cluster_labels)
        ...
        for i in range(n):
            for j in range(i + 1, n):
                if clusters[i] == clusters[j]:
                    ...
                    inter_vals.append(S[i, j])
        intra_mean = np.mean(intra_vals) if len(intra_vals) else np.nan
        inter_mean = np.mean(inter_vals) if len(inter_vals) else np.nan
        return float(intra_mean), float(inter_mean), np.array(intra_vals), np.array(inter_vals)
    ...
    if cluster_labels is not None:
        ...
        results['intra_mean_before'] = results['inter_mean_before'] = None
        results['intra_mean_after']  = results['inter_mean_after']  = None
    ...
    def dunn_index(X, labels):
        ...
        for c in clusters:
            pts = X[labels == c]
            if len(pts) > 1:
                intra_dists.append(np.max(np.linalg.norm(pts[:, None] - pts, axis=2)))
        for i, ci in enumerate(clusters):
            for cj in clusters[i+1:]:
                d = np.min(np.linalg.norm(X[labels == ci][:, None] - X[labels == cj], axis=2))
                inter_dists.append(d)
        return float(np.min(inter_dists) / np.max(intra_dists)) if intra_dists and inter_dists else np.nan

    results['dunn_before'] = safe_metric(dunn_index, emb_before, cluster_labels)
    results['dunn_after']  = safe_metric(dunn_index, emb_after, cluster_labels)

    # Variance ratio (like CH)
    ...
    results['varratio_before'] = safe_metric(variance_ratio, emb_before, cluster_labels)
    results['varratio_after']  = safe_metric(variance_ratio, emb_after, cluster_labels)
    ...
    if cluster_labels is not None:
        ...
        results['entropy'] = float(entropy_val)
    else:
        results['entropy'] = None

    # ---- 4) Welch’s t-tests for all comparable metrics ----
    metric_pairs = {
        "Silhouette": ('silhouette_before', 'silhouette_after'),
        "DB (1/x)": ('db_before', 'db_after'),
        "Inter Mean": ('inter_mean_before', 'inter_mean_after'),
        "CH": ('ch_before', 'ch_after'),
        "Dunn": ('dunn_before', 'dunn_after'),
        "Variance Ratio": ('varratio_before', 'varratio_after')
    }

    p_values = {}
    for label, (bkey, akey) in metric_pairs.items():
        bval, aval = results.get(bkey), results.get(akey)
        if bval is not None and aval is not None and not np.isnan(bval) and not np.isnan(aval):
            t_stat, p_val = stats.ttest_ind([aval], [bval], equal_var=False, alternative='greater')
    ...
    radar_metrics = {
        ...
        "Inter Mean": (results.get('inter_mean_before'), results.get('inter_mean_after')),
        "CH": (results.get('ch_before'), results.get('ch_after')),
        "Dunn": (results.get('dunn_before'), results.get('dunn_after')),
        "Variance Ratio": (results.get('varratio_before'), results.get('varratio_after')),
        "Entropy": (results.get('entropy'), results.get('entropy'))  # same before/after
    }
    ...
    if len(labels) >= 3:
        ...
        # Annotate values and p-values
        for i, angle in enumerate(angles[:-1]):
            ax.text(angle, after_vals[i]*1.05, f"{after_vals[i]:.3f}", color='orange',
                    ha='center', va='center', fontsize=9)
            ax.text(angle, before_vals[i]*0.9, f"{before_vals[i]:.3f}", color='steelblue',
                    ha='center', va='center', fontsize=9)
            p_val = p_values.get(labels[i])
            if p_val is not None and not np.isnan(p_val):
                ax.text(angle, (after_vals[i]+before_vals[i])/2,
                        f"p={p_val:.1e}", color='black', fontsize=8,
                        ha='center', va='center', bbox=dict(facecolor='white', alpha=0.6, edgecolor='gray', pad=0.2))

        ax.set_xticks(angles[:-1])
        ax.set_xticklabels(labels, fontsize=11, weight='bold')
        ax.set_title("GCL Evaluation Radar Chart (Welch’s t-test for All Metrics)", size=15, pad=20)
        ax.legend(loc='upper right', bbox_to_anchor=(1.3, 1.1))
        plt.savefig(radar_file, dpi=300, bbox_inches='tight')
        plt.close()
        print(f"✅ Radar plot saved to: {radar_file}")

    return results, results.get('silhouette_before'), results.get('silhouette_after'), results.get('db_before'), results.get('db_after')

def evaluate_gcl_no_ari(
    emb_before, emb_after,
    cluster_labels=None,
    node_labels=None,
    random_state=42,
    ...
    try:
        ...
        if len(labels) >= 3:
            ...
            # # === Add Welch’s t-test p-value annotation near but not on "Intra Mean" axis ===
            # if "Intra Mean" in labels and results.get("intra_p_val") is not None:
            #     intra_idx = labels.index("Intra Mean")
            #     angle_intra = angles[intra_idx]
            #     p_text = f"Welch’s t-test\np = {results['intra_p_val']}"

            #     # Compute offset away from axis direction
            #     offset_angle = np.deg2rad(10)  # shift annotation 10 degrees clockwise
            #     angle_annot = angle_intra + offset_angle

            #     # Compute radial offset (so it doesn't overlap the data line)
            #     radial_pos = max(after_vals[intra_idx], before_vals[intra_idx]) * 1.2

            #     # Place text slightly outside the data point
            #     ax.text(angle_annot, radial_pos, p_text,
...
def evaluate_gcl(
    ...
    Evaluate contrastive learning improvement in graph embeddings.
    Performs cluster quality analysis, intra/inter similarity statistics,
    Welch’s t-test, linear separability tests, ARI_before/ARI_after,
    and radar visualization.
    """
    ...
    # ---- 3) Clustering agreement: NMI, ARI_before, ARI_after ----
    if node_labels is not None and cluster_labels is not None:

        # After-GCL agreement
        results['NMI_after'] = normalized_mutual_info_score(node_labels, cluster_labels)
        results['ARI_after'] = adjusted_rand_score(node_labels, cluster_labels)

        # Recluster emb_before for ARI_before
        try:
            K = len(np.unique(cluster_labels))
            km = KMeans(n_clusters=K, random_state=random_state)
            cluster_labels_before = km.fit_predict(emb_before)

            results['ARI_before'] = adjusted_rand_score(node_labels, cluster_labels_before)
            results['NMI_before'] = normalized_mutual_info_score(node_labels, cluster_labels_before)

        except Exception:
            results['ARI_before'] = None
            results['NMI_before'] = None

        # Backward compatibility
        results['ARI'] = results['ARI_after']
        results['NMI'] = results['NMI_after']
    ...
    print("=== GCL Evaluation Summary ===")
    print("Silhouette before/after:", results.get('silhouette_before'), results.get('silhouette_after'))
    print("DB index before/after:", results.get('db_before'), results.get('db_after'))

    if cluster_labels is not None:
        print("Intra mean before/after:", results['intra_mean_before'], results['intra_mean_after'])
        print("Inter mean before/after:", results['inter_mean_before'], results['inter_mean_after'])
        print("Intra Welch p-value:", results.get('intra_p_val'))

    if node_labels is not None:
        print("NMI before/after:", results.get("NMI_before"), results.get("NMI_after"))
        print("ARI before/after:", results.get("ARI_before"), results.get("ARI_after"))
        print("Linear probe acc before/after:", results.get('linear_acc_before'), results.get('linear_acc_after'))
    ...
    try:
        radar_metrics = {
            ...
            "Inter Mean": (results.get('inter_mean_before'), results.get('inter_mean_after')),
            "Linear Acc": (results.get('linear_acc_before'), results.get('linear_acc_after')),
            # "ARI": (results.get('ARI_before'), results.get('ARI_after')),  # optional
        }
        ...
        if len(labels) >= 3:
            ...
            ax.plot(angles, before_vals, label='Before GCL', linewidth=2, marker='o')
            ax.fill(angles, before_vals, alpha=0.25)
            ax.plot(angles, after_vals, label='After GCL', linewidth=2, marker='o')
            ax.fill(angles, after_vals, alpha=0.25)

            for i, angle in enumerate(angles[:-1]):
                ax.text(angle, after_vals[i] * 1.05, f"{after_vals[i]:.3f}", ha='center', va='center', fontsize=10)
                ax.text(angle, before_vals[i] * 0.9, f"{before_vals[i]:.3f}", ha='center', va='center', fontsize=10)

            ax.set_xticks(angles[:-1])
            ax.set_xticklabels(labels, fontsize=12, weight='bold')
            ax.set_title("GCL Evaluation Radar Chart", size=15, pad=20)
...
def visualize_gcl_comparison_no_umap(emb_before, emb_after, cluster_labels, tsne_path, cos_path):
    """
    ...
    print(f"✅ GCL comparison visualizations saved to: {tsne_path}")

def visualize_gcl_comparison_NO_NAME_ON_AXIS(emb_before, emb_after, cluster_labels, umap_path, tsne_path, cos_path):
    """
    Compare embeddings before vs. after GCL contrastive training.
    Saves t-SNE, UMAP, and cosine similarity heatmaps.
    """
    ...
    # ---- Combine for consistent dimensionality reduction ----
    combined = np.concatenate([emb_before, emb_after], axis=0)
    n = len(emb_before)
    ...
    tsne = TSNE(n_components=2, random_state=42, perplexity=10)
    tsne_emb = tsne.fit_transform(combined)
    tsne_before, tsne_after = tsne_emb[:n], tsne_emb[n:]

    # ---- UMAP ----
    umap_model = UMAP(n_components=2, random_state=42, n_neighbors=15, min_dist=0.1)
    umap_emb = umap_model.fit_transform(combined)
    umap_before, umap_after = umap_emb[:n], umap_emb[n:]

    # ---- Color mapping ----
    cluster_labels = np.array(cluster_labels)
    colors = [CLUSTER_COLORS.get(int(c), '#808080') for c in cluster_labels]

    # ---- t-SNE plot ----
    fig, axs = plt.subplots(1, 2, figsize=(14, 6))
    axs[0].scatter(tsne_before[:, 0], tsne_before[:, 1], c=colors, s=25)
    axs[0].set_title("Before Contrastive Training (t-SNE)", fontsize=12)
    axs[0].set_xticks([]); axs[0].set_yticks([])

    axs[1].scatter(tsne_after[:, 0], tsne_after[:, 1], c=colors, s=25)
    axs[1].set_title("After Contrastive Training (t-SNE)", fontsize=12)
    axs[1].set_xticks([]); axs[1].set_yticks([])
    ...
    fig, axs = plt.subplots(1, 2, figsize=(14, 6))
    axs[0].scatter(umap_before[:, 0], umap_before[:, 1], c=colors, s=25)
    axs[0].set_title("Before Contrastive Training (UMAP)", fontsize=12)
    axs[0].set_xticks([]); axs[0].set_yticks([])

    axs[1].scatter(umap_after[:, 0], umap_after[:, 1], c=colors, s=25)
    axs[1].set_title("After Contrastive Training (UMAP)", fontsize=12)
    axs[1].set_xticks([]); axs[1].set_yticks([])

    plt.suptitle("UMAP: GCL Embeddings Before vs. After", fontsize=14)
    plt.tight_layout(rect=[0, 0.03, 1, 0.95])
    ...
    print(f"✅ GCL comparison visualizations saved to:\n  - {tsne_path}\n  - {umap_path}\n  - {cos_path}")

def visualize_gcl_comparison_no_ticles(
    emb_before, emb_after, cluster_labels,
    umap_path, tsne_path, cos_path,
    gene_names=None, max_labels=50
):
    """
    Compare embeddings before vs. after GCL contrastive training.
    Saves t-SNE, UMAP, and cosine similarity heatmaps with optional gene name labels.

    Parameters
    ----------
    emb_before : np.ndarray or torch.Tensor
        Embeddings before contrastive training.
    emb_after : np.ndarray or torch.Tensor
        Embeddings after contrastive training.
    cluster_labels : list or np.ndarray
        Cluster assignment for each gene.
    umap_path, tsne_path, cos_path : str
        Output file paths for visualizations.
    gene_names : list of str, optional
        List of gene names corresponding to embeddings.
    max_labels : int, optional
        To avoid clutter, only show up to this many axis labels.
    """
    ...
    fig, axs = plt.subplots(1, 2, figsize=(12, 6))
    sns.heatmap(cos_before, cmap='Spectral', center=0, square=True, ax=axs[0])
    axs[0].set_title("Cosine Similarity: Before GCL", fontsize=12)

    sns.heatmap(cos_after, cmap='Spectral', center=0, square=True, ax=axs[1])
    axs[1].set_title("Cosine Similarity: After GCL", fontsize=12)

    # ---- Gene name labels ----
    if gene_names is not None:
        num_genes = len(gene_names)
        if num_genes <= max_labels:
            tick_idx = np.arange(num_genes)
        else:
            step = max(1, num_genes // max_labels)
            tick_idx = np.arange(0, num_genes, step)

        selected_names = np.array(gene_names)[tick_idx]

        for ax in axs:
            ax.set_xticks(tick_idx)
            ax.set_yticks(tick_idx)
            ax.set_xticklabels(selected_names, rotation=90, fontsize=6)
            ax.set_yticklabels(selected_names, fontsize=6)
...
def visualize_gcl_comparison_ori(
    emb_before, emb_after, cluster_labels,
    umap_path, tsne_path, cos_path,
    gene_names=None, max_labels=50
):
    ...
    # ---- Normalize embeddings ----
    emb_before = torch.nn.functional.normalize(torch.tensor(emb_before), dim=1).cpu().numpy()
    emb_after  = torch.nn.functional.normalize(torch.tensor(emb_after),  dim=1).cpu().numpy()
    ...
    for ax, data, title in zip(
        ...
        ["Before Contrastive Training (t-SNE)", "After Contrastive Training (t-SNE)"]
    ):
        ax.scatter(data[:, 0], data[:, 1], c=colors, s=30, edgecolor='k', linewidth=0.3)
    ...
    fig.suptitle("t-SNE: GCL Embeddings Before vs. After", fontsize=20, y=0.98)
    ...
    for ax, data, title in zip(
        axs,
        [umap_before, umap_after],
        ["Before Contrastive Training (UMAP)", "After Contrastive Training (UMAP)"]
    ):
        ax.scatter(data[:, 0], data[:, 1], c=colors, s=30, edgecolor='k', linewidth=0.3)
        ax.set_title(title, fontsize=16, pad=8)
        ax.tick_params(axis='both', which='both', direction='out', length=6, width=1.5)
    ...
    plt.tight_layout(pad=0.2)
    plt.subplots_adjust(top=0.92)
    fig.suptitle("UMAP: GCL Embeddings Before vs. After", fontsize=20, y=0.98)
    plt.savefig(umap_path, dpi=300, bbox_inches='tight')
    plt.close()

    # ==============================================================
    # 3️⃣  Cosine Similarity Heatmaps (with shared colorbar)
    # ==============================================================
    cos_before = cosine_similarity(emb_before)
    cos_after  = cosine_similarity(emb_after)

    fig, axs = plt.subplots(1, 2, figsize=(22, 20), dpi=120)

    # Global vmin/vmax to keep the same scale
    vmin = min(cos_before.min(), cos_after.min())
    vmax = max(cos_before.max(), cos_after.max())

    heatmaps = []
    for ax, cos_mat, title in zip(
        axs,
        [cos_before, cos_after],
        ["Cosine Similarity: Before GCL", "Cosine Similarity: After GCL"]
    ):
        hm = sns.heatmap(
            cos_mat,
            cmap="Spectral",
        ...
        ax.tick_params(axis='x', direction='out', length=4, width=1, bottom=True, top=False)
        ax.tick_params(axis='y', direction='out', length=4, width=1, left=True, right=False)
    ...
    if gene_names is not None:
        ...
        for ax in axs:
            ax.set_xticks(tick_idx)
            ax.set_yticks(tick_idx)
            ax.set_xticklabels(selected_names, rotation=90, fontsize=10, ha='right')
            ax.set_yticklabels(selected_names, fontsize=10, rotation=0)

    plt.tight_layout(pad=0.3)
    plt.subplots_adjust(top=0.96)
    fig.suptitle("Pairwise Cosine Similarity Before vs. After GCL", fontsize=24, y=0.995)
    plt.savefig(cos_path, dpi=300, bbox_inches='tight')
    plt.close()
...
def visualize_gcl_comparison(
    ...
    """
    Compare embeddings before vs. after GCL contrastive training.
    Saves t-SNE, UMAP, and cosine similarity heatmaps with consistent colorbar layout.
    """

    # ==============================================================
    # 0️⃣ Normalize embeddings
    # ==============================================================
    emb_before = torch.nn.functional.normalize(torch.tensor(emb_before), dim=1).cpu().numpy()
    emb_after  = torch.nn.functional.normalize(torch.tensor(emb_after),  dim=1).cpu().numpy()

    # ==============================================================
    # 1️⃣ Combine for consistent dimensionality reduction
    # ==============================================================
    combined = np.concatenate([emb_before, emb_after], axis=0)
    n = len(emb_before)
    ...
    # ==============================================================
    # 4️⃣ Cosine Similarity Heatmaps
    # ==============================================================
    cos_before = cosine_similarity(emb_before)
    cos_after  = cosine_similarity(emb_after)
    ...
    sns.heatmap(cos_before, cmap='Spectral', center=0, square=True, vmin=vmin, vmax=vmax, ax=axs[0])
    axs[0].set_title("Cosine Similarity: Before GCL", fontsize=14)

    sns.heatmap(cos_after, cmap='Spectral', center=0, square=True, vmin=vmin, vmax=vmax, ax=axs[1])
    axs[1].set_title("Cosine Similarity: After GCL", fontsize=14)
    ...
    #         ax.set_xticks(tick_idx)
    #         ax.set_yticks(tick_idx)
    #         ax.set_xticklabels(selected_names, rotation=90, fontsize=8, ha='right')
    ...
    if gene_names is not None:
        ...
        for ax in axs:
            ax.set_xticks(tick_idx)
            ax.set_yticks(tick_idx)
            ax.set_xticklabels(selected_names, rotation=90, fontsize=6, ha='right')
            ax.set_yticklabels(selected_names, fontsize=6)

            # ✅ Force ticks to show on bottom + left only
            ax.tick_params(axis='x', length=3, width=0.5, bottom=True, top=False, labelbottom=True)
            ax.tick_params(axis='y', length=3, width=0.5, left=True, right=False, labelleft=True)

    plt.suptitle("Pairwise Similarity Before vs. After Contrastive Training", fontsize=16)
    plt.tight_layout()
    plt.savefig(cos_path, dpi=300, bbox_inches='tight')
    plt.close()
