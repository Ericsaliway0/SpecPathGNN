...
def train(hyperparams=None, data_path='gnn_embedding/data/emb', plot=True):
    ...
    def compute_embedding_quality_metrics_(embeddings, labels):
    ...
    # =====================================================
    # (1) Compute embedding quality metrics
    # =====================================================
    def compute_embedding_quality_metrics(embeddings, labels):
        """
        Compute multiple metrics for embedding quality evaluation.
        Measures clustering separation, compactness, and cosine distances.
        """
        try:
            silhouette = silhouette_score(embeddings, labels)
        except Exception:
        ...
        intra_mean = np.mean(intra_dists) if len(intra_dists) else np.nan
        inter_mean = np.mean(inter_dists) if len(inter_dists) else np.nan

        return {
            "Silhouette Score": silhouette,
            "Davies–Bouldin Index": db,
            "Intra-cluster Distance": intra_mean,
            "Inter-cluster Distance": inter_mean,
            "Intra Distribution": np.array(intra_dists),
            "Inter Distribution": np.array(inter_dists),
        }


    # =====================================================
    # (2) Visualization: Radar + Dashboard
    # =====================================================
    def visualize_gcl_embedding_quality(
        ...
        # For silhouette & inter-cluster: higher is better → normal
        # For DB & intra-cluster: lower is better → inverse normalization
        before_values = [
            normalize_metric(metrics_before["Silhouette Score"]),
            normalize_metric(metrics_before["Davies–Bouldin Index"], lower_better=True),
            normalize_metric(metrics_before["Intra-cluster Distance"], lower_better=True),
            normalize_metric(metrics_before["Inter-cluster Distance"])
        ]

        after_values = [
            normalize_metric(metrics_after["Silhouette Score"]),
            normalize_metric(metrics_after["Davies–Bouldin Index"], lower_better=True),
            normalize_metric(metrics_after["Intra-cluster Distance"], lower_better=True),
            normalize_metric(metrics_after["Inter-cluster Distance"])
        ]
        ...
        radar_path = os.path.join(
            output_dir,
            f'gcl_embedding_quality_radar_dim{out_feats}_lay{num_layers}_epo{num_epochs}.png'
        )
        ...
        plt.legend(loc='upper right', bbox_to_anchor=(1.25, 1.05), frameon=False, fontsize=11)
        plt.tight_layout()
        ...
        dashboard_path = os.path.join(
            output_dir,
            f'gcl_embedding_quality_dashboarddim_{out_feats}_lay{num_layers}_epo{num_epochs}.png'
        )

        fig, axes = plt.subplots(1, 3, figsize=(15, 5))

        # Bar chart for silhouette + DB
        bar_labels = ['Silhouette (↑)', 'Davies–Bouldin (↓)']
        before_bar = [metrics_before['Silhouette Score'], metrics_before['Davies–Bouldin Index']]
        after_bar = [metrics_after['Silhouette Score'], metrics_after['Davies–Bouldin Index']]
        ...
        axes[0].set_xticks(x)
        axes[0].set_xticklabels(bar_labels, rotation=15, ha='right')
        axes[0].set_ylabel('Score')
        axes[0].set_title('Clustering Metrics')
        axes[0].legend()

        # Intra violin (smaller = better)
        sns.violinplot(
            data=[metrics_before["Intra Distribution"], metrics_after["Intra Distribution"]],
        ...
        # Inter violin (larger = better)
        sns.violinplot(
            data=[metrics_before["Inter Distribution"], metrics_after["Inter Distribution"]],
            palette=["#1f77b4", "#ff7f0e"], ax=axes[2]
        )
        axes[2].set_xticks([0, 1])
        axes[2].set_xticklabels(['Before GCL', 'After GCL'])
        axes[2].set_ylabel('Cosine Distance')
        axes[2].set_title('Inter-cluster Distance (↑)')
...
def visualize_gcl_comparison(emb_before, emb_after, cluster_labels, tsne_path, cos_path):
    """
